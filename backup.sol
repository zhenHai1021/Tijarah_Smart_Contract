    ...
    */
    ...
    function addProduct(
     string memory _productName,
        ...
        require(checkMedicineID(_medicineID), "Medicine OR Supplier Not Existed.");
    ...
    */
        ...
        returns (string memory)
    {
        ...
        verifyOperation.VerifiedManufacturer[] memory vm = verifyOp
            .getAllVManufacturer(getOwnerAddress());
        for (uint256 i = 0; i < vm.length; i++) {
             if (keccak256(abi.encodePacked((vm[i].manufacturerAddress))) == keccak256(abi.encodePacked((_manufacturerAddress)))) {
...
function getManufacturerID(address _manufacturerAddress)
    ...
    */
    ...
    function checkMedicineID(string memory _medicineID) internal view returns (bool) {
    verifyOperation.Medicine[] memory med = verifyOp.getAllMedicines(getOwnerAddress());

    for (uint256 i = 0; i < med.length; i++) {
        if (keccak256(abi.encodePacked((med[i].medicineID))) == keccak256(abi.encodePacked((_medicineID)))) {
            return true;
        }
    ...
    {
         verifyOperation.VerifiedManufacturer[] memory vm = verifyOp.getAllVManufacturer(getOwnerAddress());
        for (uint256 i = 0; i < vm.length; i++) {
